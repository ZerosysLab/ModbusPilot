...
Click ➕ Channel in the left Resource Explorer toolbar (or right-click the root node).
Select the communication mode in the popup window:
Serial (RTU): Select COM port, Baud Rate, and other parameters.
Ethernet (TCP): Enter target IP and Port (default 502).
Click OK, and a new channel node will appear in the tree menu.
1.2 Add Device
Select the channel created in the previous step.
Click ➕ Device in the toolbar (or right-click the channel node).
Set the Slave ID and Device Name.
...
Ensure points (tags) have been added to the device.
Expand the device in the left tree to find specific Tag Nodes.
Hold the left mouse button and Drag the tag to the blank area on the right.
Release the mouse. The system will automatically generate a widget based on the point type:
