...
## 📥 下载即用
**ModbusPilot 为绿色独立版 (Self-Contained)，解压即用，无需安装 .NET 8 环境。**

1. [👉 点击前往下载最新版](https://gitee.com/ZerosysLab/ModbusPilot/releases)
2. 解压 `ModbusPilot_vx.x.x.zip`。
3. 双击 `ModbusPilot.exe`（若遇拦截，请参考下方的【故障排查】说明）。

---

### 🛠️ 故障排查与运行说明

> **💡 如果双击后弹出“Windows 已保护你的电脑”：**
> 由于 ModbusPilot 采用高性能混淆加密且尚未支付高昂的微软数字签名年费，会触发 Windows SmartScreen 误报。
> *   **解决方法**：点击窗口中的 **“更多信息”** -> 点击右下角出现的 **“仍要运行”** 即可正常启动。
>
> **💡 如果双击完全没有反应（无报错、无界面）：**
> 这种情况通常是软件已被 **360、火绒或 Windows Defender** 静默拦截。
> *   **原因**：混淆后的代码特征易被杀软误判。
> *   **解决方法**：请在杀毒软件中将本软件目录设为 **“信任/白名单”**，或暂时关闭杀软后再试。
> *   **承诺**：本软件绿色无毒，除必要的软件激活授权信息需存入系统配置外，所有行为仅限 Modbus 通讯与数据展示。
